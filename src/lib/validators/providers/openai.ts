/**
 * OpenAIValidator — placeholder for cross-model interpretation checks.
 *
 * SCAFFOLD ONLY. Returns mocked output regardless of env state when the
 * gates would let a real call through. The real OpenAI API call is
...
const MOCK_RESPONSE: NormalizedValidationOutput = {
  ...
  cited_sources: [],
  raw_summary: "[MOCK] OpenAI validator placeholder — real API call deferred",
  error: null,
};
...
export const OpenAIValidator: AiValidator = {
  ...
  async validateBusiness(
    ...
    // TODO(prod-integration): replace MOCK_RESPONSE with a real OpenAI
    // API call. Read OPENAI_API_KEY from process.env. Map the response
